    ...
    void pubCabinetParam()
{
    if (!is_calculate)
    {
        nlohmann::json cabinet_data_json;

        // 对 cabinet_wdh 保留两位小数
        cabinet_data_json["cabinet_wdh"] = {
            std::round(cabinet_width * 100.0) / 100.0,
            std::round(cabinet_depth * 100.0) / 100.0,
            std::round(cabinet_height * 100.0) / 100.0
        };

        nlohmann::json cabinet_points_json;
        for (int i = 0; i < horizontal_cabinet_num; i++)
        {
            for (int j = 0; j < vertical_cabinet_num; j++)
            {
                ...
                std::vector<double> point = {
                    std::round((cabinet_origin[0] + i * (cabinet_width + vertical_partition_width)) * 100.0) / 100.0,
                    std::round(cabinet_origin[1] * 100.0) / 100.0,
                    std::round((cabinet_origin[2] + j * (cabinet_height + horizontal_partition_height)) * 100.0) / 100.0
                };

                cabinet_points_json[key] = point;
            }
        }
        cabinet_data_json["cabinet_points"] = cabinet_points_json;

        cabinet_data.data = cabinet_data_json.dump();
        is_calculate = true;
    }
    cabinet_points_pub_->publish(cabinet_data);
}
